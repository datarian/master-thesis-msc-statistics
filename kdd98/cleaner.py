...
from kdd98.data_loader import KDD98DataLoader, ModelPersist
...
class Cleaner:
    ...
    def __init__(self, data_loader=KDD98DataLoader(), fit=False):
        self.dl = data_loader
        self.fit = fit
        self.mp = ModelPersist()
        assert(self.dl.raw_data_file_name in [App.config('learn_file_name'), App.config('validation_file_name')])
    ...
    def clean(self):
        ...
        ordinal_transformer = ColumnTransformer([
            ("order_ordinals",
            OrdinalEncoder(mapping=self.dl.ordinal_mapping_mdmaud,handle_unknown='ignore'),
            ['MDMAUD_R','MDMAUD_A']),
            ("order_multibytes",
            OrdinalEncoder(mapping=self.dl.ordinal_mapping_rfa,handle_unknown='ignore'),
        ...
        self.dataset.ZIP = self.dataset.ZIP.str.replace('-', '').replace([' ', '.'], np.nan).astype('int64')
        ...
        self.dataset[['MDMAUD_R', 'MDMAUD_F', 'MDMAUD_A']] = self.dataset.loc[:,['MDMAUD_R', 'MDMAUD_F', 'MDMAUD_A']].replace('X', np.nan)

        binarys = binary_transformers.fit_transform(self.dataset)
        self.dataset = ut.update_df_with_transformed(self.dataset, binarys, binary_transformers)
        multibytes = multibyte_transformer.fit_transform(self.dataset)
        self.dataset = ut.update_df_with_transformed(self.dataset, multibytes, multibyte_transformer, drop=self.dl.nominal_features, new_dtype="category")
        domains = domain_transformer.fit_transform(self.dataset)
        self.dataset = ut.update_df_with_transformed(self.dataset, domains, domain_transformer)
        ordinals = ordinal_transformer.fit_transform(self.dataset)
        self.dataset = ut.update_df_with_transformed(self.dataset, ordinals, ordinal_transformer)
