...
Each $i$ represents one example. We have $m = p-3 = `r ncol(py$raw_data$data)-2`$ explanatory features, two targets and one unique identifier for each example.
...
(ref:rfa-alltime-legend) Analysis of all-time RFA values. *Recency* is the time in months since the last donation, *Frequency* the average number of donations per year and *Amount* the average yearly donation amount.
